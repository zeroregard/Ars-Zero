...
/**
 * Spell behaviour: lightning casting circle + lightning voxel hover for 2s, then Push toward target.
 */
public class LightningVoxelPushSpellBehaviour implements MobSpellBehaviour {
    ...
    public boolean run(Mob caster, LivingEntity target) {
        ...
        if (circleEntity != null) {
            ...
            circleEntity.setCurrentSchoolId("school_lightning");
        ...
        for (Vec3 pos : voxelPositions) {
            LightningVoxelEntity voxel = new LightningVoxelEntity(serverLevel, pos.x, pos.y, pos.z, VOXEL_LIFETIME_TICKS);
