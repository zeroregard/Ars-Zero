...
/**
 * Spell behaviour: ice casting circle + ice voxel hover for 2s, then Push toward target.
 */
public class IceVoxelPushSpellBehaviour implements MobSpellBehaviour {
    ...
    public boolean run(Mob caster, LivingEntity target) {
        ...
        if (circleEntity != null) {
            ...
            circleEntity.setCurrentSchoolId("school_water");
        ...
        for (Vec3 pos : voxelPositions) {
            IceVoxelEntity voxel = new IceVoxelEntity(serverLevel, pos.x, pos.y, pos.z, VOXEL_LIFETIME_TICKS);
