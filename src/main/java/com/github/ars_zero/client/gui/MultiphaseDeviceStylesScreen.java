    ...
    private void initHotkeySlots() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            GuiSpellSlot slot = new GuiSpellSlot(bookLeft + 281, bookTop - 1 + 15 * (i + 1), i, name, (b) -> {
                ...
                if (staffStack != null && !staffStack.isEmpty() && staffStack.getItem() instanceof com.github.ars_zero.common.item.AbstractMultiphaseHandheldDevice) {
                    freshStack = staffStack;
                } else if (hand != null) {
                    freshStack = player.getItemInHand(hand);
                    if (!(freshStack.getItem() instanceof com.github.ars_zero.common.item.AbstractMultiphaseHandheldDevice)) {
                        freshStack = ItemStack.EMPTY;
                    }
                } else {
                    ItemStack mainStack = player.getMainHandItem();
                    if (mainStack.getItem() instanceof com.github.ars_zero.common.item.AbstractMultiphaseHandheldDevice) {
                        freshStack = mainStack;
                    } else {
                        ItemStack offStack = player.getOffhandItem();
                        if (offStack.getItem() instanceof com.github.ars_zero.common.item.AbstractMultiphaseHandheldDevice) {
                            freshStack = offStack;
                        }
                    }
                }
                ...
                if (freshStack.isEmpty() && isForCirclet) {
                    freshStack = top.theillusivec4.curios.api.CuriosApi.getCuriosHelper()
                        .findEquippedCurio(
                            equipped -> equipped.getItem() instanceof com.github.ars_zero.common.item.AbstractMultiphaseHandheldDevice,
                            player
                        )
                ...
                if (freshStack.isEmpty() || !(freshStack.getItem() instanceof com.github.ars_zero.common.item.AbstractMultiphaseHandheldDevice)) {
                    return;
                }
